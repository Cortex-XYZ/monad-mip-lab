...
1. **`forge test` tracks reserve dips in the tested Monad Foundry v1.7.1 setup.** A delegated account funded with 10.5 MON before the isolated test transaction reports `false → true` after a 1 MON debit; the guard reverts and unwinds the transfer. The recorded `1.5.0-stable-monad` experiment exposed the precompile but returned `false → false` under a different project configuration. The toolchain version and project configuration are therefore part of the regression contract.
...
| 9 | **Monad Foundry reserve tracking depends on the tested toolchain and project configuration.** The recorded `1.5.0-stable-monad` setup exposed the precompile but did not populate the tracker in `forge test`; the configured `v1.7.1-monad-v1.0.0` setup passes the delegated-account `false → true` regression. The existing records do not isolate which change produced the different result. | Pin the tested release and configuration, then run the real-dip Forge regression before accepting an upgrade. |
